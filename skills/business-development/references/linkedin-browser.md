...
1. **Dedicated agent-controlled browser profile** (`openclaw` or a dedicated profile like `linkedin`)
   ...
   - No attach prompt to Ryan's personal Chrome session
2. **Remote CDP browser** (Browserbase / Browserless / remote node-hosted browser)
   - Use when the browser should live outside Ryan's Mac or when always-on remote control matters
3. **`user` profile / Chrome attach**
   - Fallback only when Ryan's existing signed-in Chrome state is required *and* Ryan is at the computer to approve the attach prompt
...
If Ryan wants browser control decoupled from the Mac, use a remote CDP profile:
- Browserbase
- Browserless
...
When the sheet is missing a LinkedIn URL and Ryan has asked to add or verify a lead:

1. Use the signed-in agent-controlled browser profile (`openclaw` by default on this Mac).
2. Search LinkedIn by:
   - full name
   - full name + company
   - full name + company + location
3. Prefer a clear personal profile match over company pages, search-result URLs, or guessed slugs.
4. If a confident personal profile does not appear after a reasonable search, leave the LinkedIn field blank and record that no clear profile was found rather than bouncing the verification back to Ryan.
5. If browser control is temporarily unavailable, record that explicitly in `R2's Notes` and retry later instead of pretending Ryan needs to do the search.
...
- `LI accepted` -> set to `Yes` only on clear evidence, preferably `1st`
- Leave it unchanged for `Pending`, `2nd`, `3rd`, or ambiguous states
- Put operational notes in `R2's Notes`, not `Ryan's Notes`
...
3. If Ryan needs off-machine reliability, move the same workflow to a remote CDP profile.
4. Only use the Chrome attach path when Ryan is present and explicitly wants reuse of his live personal browser session.
